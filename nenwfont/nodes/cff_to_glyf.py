...
def otf_to_ttf(ttFont, post_format=POST_FORMAT, **kwargs):
    ...
    try:
        ...
        post.formatType = 3
        print("Dropping glyph names, they do not fit in 'post' table.")

    ttFont.sfntVersion = "\000\001\000\000"


class CFFToGlyfNode(Node):
    name = 'cff_to_glyf'
    updates_fonts = True

    def transform(self, input_fonts, options):
        max_err = options.get('max_err', MAX_ERR)
        post_format = options.get('post_format', POST_FORMAT)
